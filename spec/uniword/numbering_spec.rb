...
RSpec.describe 'Numbering Feature' do
  describe Uniword::NumberingLevel do
    describe '#initialize' do
      it 'creates a level with default values' do
        level = Uniword::NumberingLevel.new
    ...
    describe '#numbering_text' do
      it 'returns bullet character for bullet format' do
        level = Uniword::NumberingLevel.new(format: 'bullet', text: '•')
        expect(level.numbering_text).to eq('•')
      end

      it 'returns default decimal text' do
        level = Uniword::NumberingLevel.new(level: 0, format: 'decimal')
        expect(level.numbering_text).to eq('%1.')
      end

      it 'returns custom text if provided' do
        level = Uniword::NumberingLevel.new(level: 0, format: 'decimal', text: '%1)')
        expect(level.numbering_text).to eq('%1)')
      end
    end
  end

  describe Uniword::NumberingDefinition do
    describe '#initialize' do
    ...
    describe '.decimal' do
      it 'creates decimal numbering definition' do
        defn = Uniword::NumberingDefinition.decimal(abstract_num_id: 1)
    ...
    describe '.bullet' do
      it 'creates bullet numbering definition' do
        defn = Uniword::NumberingDefinition.bullet(abstract_num_id: 2)
    ...
    describe '.roman' do
      it 'creates upper roman numbering' do
        defn = Uniword::NumberingDefinition.roman(abstract_num_id: 3, upper: true)
        expect(defn.levels[0].format).to eq('upperRoman')
      end

      it 'creates lower roman numbering' do
        defn = Uniword::NumberingDefinition.roman(abstract_num_id: 4, upper: false)
    ...
    describe '.letter' do
      it 'creates upper letter numbering' do
        defn = Uniword::NumberingDefinition.letter(abstract_num_id: 5, upper: true)
        expect(defn.levels[0].format).to eq('upperLetter')
      end

      it 'creates lower letter numbering' do
        defn = Uniword::NumberingDefinition.letter(abstract_num_id: 6, upper: false)
        expect(defn.levels[0].format).to eq('lowerLetter')
      end
    end
  end

  describe Uniword::NumberingInstance do
    describe '#initialize' do
      it 'creates instance with valid IDs' do
        instance = Uniword::NumberingInstance.new(num_id: 1, abstract_num_id: 0)
    ...
    describe '#uses_definition?' do
      it 'returns true for matching definition' do
        instance = Uniword::NumberingInstance.new(num_id: 1, abstract_num_id: 5)
        definition = Uniword::NumberingDefinition.new(abstract_num_id: 5)
        expect(instance.uses_definition?(definition)).to be true
      end

      it 'returns false for non-matching definition' do
        instance = Uniword::NumberingInstance.new(num_id: 1, abstract_num_id: 5)
        definition = Uniword::NumberingDefinition.new(abstract_num_id: 3)
        expect(instance.uses_definition?(definition)).to be false
      end
    end
  end

  describe Uniword::NumberingConfiguration do
    describe '#initialize' do
    ...
    describe '#add_definition' do
      ...
      it 'respects provided abstract_num_id' do
        config = Uniword::NumberingConfiguration.new
        defn = Uniword::NumberingDefinition.new(abstract_num_id: 5)
  ...
  describe 'Document Integration' do
    it 'initializes with numbering configuration' do
      ...
      expect(doc.numbering_configuration).to be_a(Uniword::NumberingConfiguration)
    end
