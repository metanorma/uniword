...
RSpec.describe Uniword::Accessibility::AccessibilityChecker do
  ...
  describe "#initialize" do
    ...
    it "uses custom config file when provided" do
      ...
      config_content = {
        profiles: {
          custom: {
            ...
            level: "Test",
            rules: {
              image_alt_text: { enabled: true, wcag_criterion: "1.1.1",
                                level: "A" },
            },
          },
        },
      ...
      checker = described_class.new(profile: :custom,
      ...
      config_content = {
        profiles: {
          all_disabled: {
            ...
            level: "Test",
            rules: {
              image_alt_text: { enabled: false, wcag_criterion: "1.1.1",
                                level: "A" },
              heading_structure: { enabled: false, wcag_criterion: "1.3.1",
                                   level: "A" },
              table_headers: { enabled: false, wcag_criterion: "1.3.1",
                               level: "A" },
              list_structure: { enabled: false, wcag_criterion: "1.3.1",
                                level: "A" },
              reading_order: { enabled: false, wcag_criterion: "1.3.2",
                               level: "A" },
              color_usage: { enabled: false, wcag_criterion: "1.4.1",
                             level: "A" },
              contrast_ratio: { enabled: false, wcag_criterion: "1.4.3",
                                level: "AA" },
              document_title: { enabled: false, wcag_criterion: "2.4.2",
                                level: "A" },
              descriptive_headings: { enabled: false, wcag_criterion: "2.4.6",
                                      level: "AA" },
              language_specification: { enabled: false,
                                        wcag_criterion: "3.1.1", level: "A" },
            },
          },
        },
      }
  ...
  describe "different profiles" do
    let(:document) do
      doc = double("Document")
      allow(doc).to receive_messages(images: [], tables: [], paragraphs: [])
      doc
    end
