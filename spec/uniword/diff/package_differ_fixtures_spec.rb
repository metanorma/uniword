...
RSpec.describe Uniword::Diff::PackageDiffer, "Word repair stats analysis" do
  ...
  FIXTURE_PAIRS = {
    "01_single_word" => %w[spec/fixtures/01_single_word.docx
                           spec/fixtures/01_single_word-repaired.docx],
    "02_two_words" => %w[spec/fixtures/02_two_words.docx
                         spec/fixtures/02_two_words-repaired.docx],
    "03_two_paragraphs" => %w[spec/fixtures/03_two_paragraphs.docx
                              spec/fixtures/03_two_paragraphs-repaired.docx],
    "04_with_empty_para" => %w[spec/fixtures/04_with_empty_para.docx
                               spec/fixtures/04_with_empty_para-repaired.docx],
    "05_longer_text" => %w[spec/fixtures/05_longer_text.docx
                           spec/fixtures/05_longer_text-repaired.docx],
    "06_cjk_text" => %w[spec/fixtures/06_cjk_text.docx
                        spec/fixtures/06_cjk_text-repaired.docx],
  }.freeze
  ...
  def aggregate
    all_results.filter_map do |label, r|
      [label, yield(label, r)] if yield(label, r)
    end
  end
  ...
  describe "Canon semantic comparison" do
    it "webSettings.xml is always canon-equivalent (only whitespace)" do
      all_results.each do |label, result|
        ...
        expect(ws).not_to be_nil,
                          "#{label}: webSettings.xml not in modified parts"
      ...
      all_results.each do |label, result|
        always_different.each do |part_name|
          ...
          expect(part).not_to be_nil,
                              "#{label}: #{part_name} not in modified parts"
    ...
    it "canon_summary is set on every canon-different part" do
      all_results.each do |label, result|
        result.modified_parts.select do |p|
          p.canon_equivalent == false
        end.each do |p|
  ...
  describe "theme1.xml expansion" do
    it "Word always expands the minimal theme to a full theme" do
      all_results.each do |label, result|
        theme = result.modified_parts.find do |p|
          p.name == "word/theme/theme1.xml"
        end
  ...
  describe "styles.xml cleanup" do
    it "Word always removes empty kern/sz/szCs elements from docDefaults" do
      all_results.each do |label, result|
        ...
        element_changes = styles.changes.select do |c|
          c.category == :element_count
        end
        removed = element_changes.select do |c|
          c.description.match?(/kern|sz|szCs/)
        end
  ...
  describe "fontTable.xml cleanup" do
    ...
    it "sig element count always decreases" do
      all_results.each do |label, result|
        ft = result.modified_parts.find { |p| p.name == "word/fontTable.xml" }
        sig_change = ft.changes.find do |c|
          c.description.match?(/sig: \d+ -> \d+/)
        end
        expect(sig_change).not_to be_nil,
                                  "#{label}: no sig element count change"
  ...
  describe "settings.xml changes" do
    it "Word always adds rsid elements" do
      all_results.each do |label, result|
        settings = result.modified_parts.find do |p|
          p.name == "word/settings.xml"
        end
        rsid_change = settings.changes.find do |c|
          c.description.match?(/rsid: \d+ -> \d+/)
        end
    ...
    it "Word always removes doNotDisplayPageBoundaries" do
      all_results.each do |label, result|
        ...
        dndpb = settings.changes.find do |c|
          c.description.include?("doNotDisplayPageBoundaries")
        end
