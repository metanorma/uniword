...
* DOCX validation and verification (`uniword verify` with 3-layer pipeline)
* Prevention layer (auto-fixes footnote/endnote cross-part invariants)
* Open-source resources (23 color schemes, 25 font schemes, 30-locale document elements)
...
=== Docx::Package - Core Architecture
...
**Auto-fixes applied before writing:**
...
The reconciler runs in `Docx::Package#to_zip_content` before the serialization phase. It is NOT an extension point -- it enforces built-in DOCX invariants. For customizable validation, use `uniword verify` or `Uniword::Validation::Rules`.

== Open-Source Resources

Uniword ships with OFL-licensed open-source resources that replace proprietary Microsoft Word assets:
...
**30 locales supported:** ar, cs, da, de, el, en, en-GB, es, es-MX, fi, fr, fr-CA, he, hu, id, it, ja, ko, nl, no, pl, pt, pt-PT, ru, sk, sv, th, tr, zh-CN, zh-TW

Each locale has 8 document element types: bibliographies, cover_pages, equations, footers, headers, table_of_contents, tables, watermarks.

See link:docs/RESOURCE_AUTHORING.md[Resource Authoring Guide] and link:docs/RESOURCE_MAPPING.md[Resource Mapping] for details.
...
Comprehensive 3-layer DOCX verification:
...
**Custom rules** can be registered via `Uniword::Validation::Rules.register(MyRule)`.
